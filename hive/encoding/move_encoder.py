...
Policy layout: 7 conceptual channels x grid_size x grid_size = 7*G*G flat vector.

  [0 .. 5*G*G)      placement head: piece_type (0-4) × dest
  [5*G*G .. 6*G*G)  movement source head
  [6*G*G .. 7*G*G)  movement destination head

MCTS scoring:
  Placement (type, dest):  prior = policy[type * G² + dest_cell]
  Movement  (src,  dest):  prior = policy[5*G² + src_cell] + policy[6*G² + dest_cell]
"""

from .board_encoder import DEFAULT_GRID_SIZE, GRID_SIZE

NUM_POLICY_CHANNELS = 7      # 5 place + 1 src + 1 dst
NUM_PLACE_CHANNELS = 5       # placement head channels
POLICY_SIZE = NUM_POLICY_CHANNELS * GRID_SIZE * GRID_SIZE  # default 3,703
